...
import com.ainnotate.aidas.config.KafkaProperties;
...
class AinnotateserviceKafkaResourceIT {
    ...
    void setup() {
        ...
        AinnotateserviceKafkaResource kafkaResource = new AinnotateserviceKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/ainnotateservice-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/ainnotateservice-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
