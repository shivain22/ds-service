				...
				if (uploadDto.getConsentFormUrl() != null && uploadDto.getConsentFormUrl().trim().length() > 0) {
					...
					try {
							...
							*/
							...
							List<UploadMetaData> metadatas = uploadMetaDataRepository.getAllUploadMetaDatasProjectProperty(upload.getId(),uploadMetaDataKeys); 
							metadatas.addAll(uploadMetaDataRepository.getAllUploadMetaDatasObjectProperty(upload.getId(),uploadMetaDataKeys)); 
						...
						System.out.println(uploadDto.getObjectId()+"==="+uploadDto.getUserVendorMappingObjectMappingId()+e.getMessage());
		...
		if (totalPendings == 0) {
								...
								if (batchStatus1 != null && batchStatus1.size() > 0) {
									...
									if (batchStatus2 != null && batchStatus2.size() > 0) {
										projectRepository.addTotalApprovedSubtractTotalPendingNonGrouped(
												batchStatus2.get(0).getProjectId(),
												batchStatus2.get(0).getTotalApproved(),
										...
										if (batchStatus3 != null && batchStatus3.size() > 0) {
											objectRepository.addTotalApprovedSubtractTotalPendingNew(
								...
								if (qpm.getProject().getQcLevels().equals(qpm.getQcLevel())) {
									userVendorMappingObjectMappingRepository
											.addTotalRejectedAndSubtractTotalPendingAddTotalRequiredNonGrouped(
													usfqcf.getUvmomId(), usfqcf.getTotalRejected(),
													usfqcf.getTotalPending());
									Integer totalRejectedForObject = objectRepository
											.getTotalRejectedForObjectFromUploads(usfqcf.getObjectId());
									objectRepository
											.addTotalRejectedAndSubtractTotalPendingAddTotalRequiredForNonGrouped(
													usfqcf.getObjectId(), usfqcf.getTotalRejected(),
													usfqcf.getTotalPending(), totalRejectedForObject);
								} else {
									objectRepository
											.addTotalRejectedAndSubtractTotalPendingAddTotalRequiredForNonGroupedBeforeLastLevel(
													usfqcf.getObjectId(), usfqcf.getTotalRejected(),
					...
					if (qpm.getProject().getQcLevels().equals(qpm.getQcLevel())) {
						if (qpm.getProject().getAutoCreateObjects().equals(AidasConstants.AUTO_CREATE_OBJECTS)) {
							...
							if (batchStatus1 != null && batchStatus1.size() > 0) {
								...
								if (batchStatus2 != null && batchStatus2.size() > 0) {
									projectRepository.addTotalApprovedSubtractTotalPendingNonGroupedApproved(
			...
			if (batchInfo != null) {
				...
				if (batchInfo.getTotalUploaded().equals(batchInfo.getTotalApproved())) {
					...
					if (qpm.getProject().getAutoCreateObjects().equals(AidasConstants.AUTO_CREATE_OBJECTS)) {
						/*
						 * List<Long[]> getUvmomIdsSummary = uploadQcProjectMappingBatchInfoRepository.
						 * getBatchWiseReportForGroupedProjectGroupedByUvmomToGetRejectionCompleted(
						 * batchNumber,qpm.getProject().getId()); int i=0;
						 * for(Long[]l:getUvmomIdsSummary) { if(l[3]>0) { i++; } }
						 * if(i==getUvmomIdsSummary.size()) { for(Long[] l:getUvmomIdsSummary) {
						 * UserVendorMappingObjectMapping uvmom=
						 * userVendorMappingObjectMappingRepository.getById(l[0]);
						 * objectRepository.addTotalApprovedSubtractTotalPending(uvmom.getObject().getId
						 * (),l[2].intValue());
						 * userVendorMappingObjectMappingRepository.addTotalApprovedSubtractTotalPending
						 * (l[0],l[2].intValue(),l[4].intValue());
						 * uploadQcProjectMappingBatchInfoRepository.
						 * updateUqpmbiUploadFinalLevelWhenAllInBatchIsRejectedWithMixedApprovalForGrouped
						 * (qcProjectMappingId,batchNumber); } }
						 */
					}
				}
				return ResponseEntity.ok().body(true);
