...
public interface AidasObjectPropertyRepository extends JpaRepository<AidasObjectProperty, Long> {
    ...
    @Query(value = "select aidas_properties_id from aidas_object_property where aidas_properties_id not in (select aidas_properties_id from aidas_project_property where aidas_project_id=?1)",nativeQuery = true)
    List<AidasObjectProperty> findAllUncommonAidasObjectPropertyForMetadata(Long projectId);

    @Query(value = "select count(*) from aidas_object_property aop where aop.aidas_properties_id not in (select aidas_properties_id from aidas_project_property where aidas_project_id=?1) and aop.optional=1",nativeQuery = true)
    Long findAllUncommonMandatoryAidasObjectPropertyForMetadata(Long projectId);
