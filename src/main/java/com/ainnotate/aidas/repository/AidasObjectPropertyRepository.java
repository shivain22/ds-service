...
public interface AidasObjectPropertyRepository extends JpaRepository<AidasObjectProperty, Long> {
    ...
    @Query(value = "select aidas_properties_id from aidas_object_property aop where aop.aidas_object_id=?1 and aop.aidas_properties_id not in (select aidas_properties_id from aidas_project_property where aidas_project_id=?2)",nativeQuery = true)
