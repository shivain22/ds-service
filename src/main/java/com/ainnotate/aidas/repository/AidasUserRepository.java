...
public interface AidasUserRepository extends JpaRepository<AidasUser, Long> {
    ...
    @Query(value = "select au.* from aidas_user au, aidas_user_aidas_authority auaa where au.aidas_vendor_id=?1 and au.id=auaa.aidas_user_id and auaa.aidas_authority_id=5",nativeQuery = true)
    List<AidasUser> findAllByAidasVendor_Id(Long aidasVendorId);
